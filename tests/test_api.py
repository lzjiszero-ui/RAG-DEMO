...
import main
from rag import QueryPlan, SearchHit
...
def test_ask_stream_reports_real_pipeline_steps(monkeypatch) -> None:
    ...
    monkeypatch.setattr(main, "plan_queries", lambda *args: QueryPlan("全部", ["rewritten query", "keyword query"]))
    monkeypatch.setattr(main, "retrieve_multi_query_candidates", lambda queries, category: [candidate])
    ...
    monkeypatch.setattr(main, "verify_citations", lambda answer, hits: {"enabled": True, "status": "passed", "claims": []})
...
def test_second_turn_receives_first_turn_history(monkeypatch) -> None:
    ...
    monkeypatch.setattr(main, "plan_queries", lambda question, rewritten, history, category, available: QueryPlan(category, [rewritten]))
    monkeypatch.setattr(main, "retrieve_multi_query_candidates", lambda queries, category: [candidate])
    monkeypatch.setattr(main, "rerank_candidates", lambda question, candidates: [hit])
